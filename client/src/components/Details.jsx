          ...
          <div>
            ...
            {recipeDetails.dish ? (
              <div>
                ...
                {recipeDetails.dish?.map((d) => {
                  return <p key={d}>- {d[0].toUpperCase() + d.slice(1)} </p>;
                })}
              </div>
            ) : (
            ...
            {recipeDetails.summary ? (
              <div>
                ...
                <p className={style.area1}>
                  {recipeDetails.summary?.replace(/<[^>]*>/g, "")}
                </p>
              </div>
            ) : (
              <h5 className={style.notFound}>
                This recipe does not have summary.
              </h5>
            )}
            ...
            {recipeDetails.score ? (
              ...
              <h5 className={style.notFound}>
                This recipe does not have score.
              </h5>
            )}
                ...
                <ul>
                  {Array.isArray(recipeDetails.steps) ? (
                    recipeDetails.steps.map((s) => {
                      return (
                        <p className={style.area2} key={s.number}>
                          {s.number}: {s.step}
                        </p>
                      );
                    })
